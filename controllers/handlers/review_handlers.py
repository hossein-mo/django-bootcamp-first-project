...
class SubmitMovieRate(AbstractHandler):
    """
    Handler for submiting movie rate
    ...
    def handle(self, data: dict) -> dict | None:
        ...
        mrate = MovieRate.fetch_obj(
            where=f"{MovieRate.movie_id}={data['movie_id']} AND {MovieRate.user_id} = {data['user'].id}"
        )
...
class SubmitTheaterRate(AbstractHandler):
    """
    Handler for submiting theater rate
    ...
    def handle(self, data: dict) -> dict | None:
        ...
        trate = TheaterRate.fetch_obj(
            where=f"{TheaterRate.theater_id}={data['theater_id']} AND {TheaterRate.user_id} = {data['user'].id}"
        )
        if data["rate"] > 5 or data["rate"] < 1:
            raise UnvalidRate
        if trate:
...
class WriteComment(AbstractHandler):
    ...
    def handle(self, data: dict) -> dict | None:
        ...
        if data["parent_id"] == 0:
            data["parent_id"] = None
        comm = Comment.create_new(
            user.id, data["movie_id"], data["parent_id"], data["text"]
        )
        print(comm.info())
        data["comment"] = comm
        if self._next_handler:
